...
class ValueSelector:
    ...
    def send_command(self):
        if self.command_type == DrumCommandTypes.SCAN_TIME:
            DrumCommands.send_new_scan_time(self.current_value, self.serial_connection, self.console)
        elif self.command_type == DrumCommandTypes.THRESHOLD:
            self.console.log("Error! Trying to update threshold from a base ValueSelector")
